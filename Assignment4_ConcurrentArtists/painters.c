...
//This method is to check for the same colours
int sameColors(int colors[54][3], int color[3]) {
    for (int i = 0; i < 54; i++) {
    ...
    for (i = 0; i < 5000; ++i) {
        ...
        if (canvas[painter->x][painter->y].r == 255 &&
            ...
            if (pthread_mutex_trylock(&canvas[painter->x][painter->y].lock) == 0) {
                ...
                //Copied from the else statement below
