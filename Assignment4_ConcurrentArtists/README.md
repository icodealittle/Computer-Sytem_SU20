...
- Did you use any external resources? (Cite them below)
 ...
 - https://www.youtube.com/watch?v=xDVC3wKjS64&t=34s
...
Starvation is normally occured is when there's deadlock in that it, which the process to freeze. Two or more processes become deadlocked and each of them isn't doing anythig while waiting for a resource occupied by another program in the same set. If that the case, then we would try to use trylock function rather than lock function. If the mutex is already locked, then the routine will return immediately with an error code. This routine may help to prevent deadlock from happening.

On contrary, when a process in a starvation because it is waiting for any resources that would continously given to other process. If there are lots of artists, then the allocation of recourse may be unfair. If so, the starvation might be occur. A possibility to prevent starvation to occurs is to use a scheduling algorithm with priority queue that also uses the aging technique. It is a technique that gradually increas the priority of processes that have been waiting for a long time.
...
Trylock vs. Lock:
	- Trylock: if the mutex is already locked, then the routine will return immediately with a "busy" error code. This routine may be useful in preventing deadlock conditions, as in a priority-inversion situation (somewhat discussed above – Part 3).
	- Lock: If the mutex is already locked by another thread, then the call will block the calling thread until the mutex is unlocked.

o memory in 4 segments:
	- code(text)
	- static or global variable
	- stack or queue of any function that being call and their local variable
	- heap memory
	
Dynamic memory allocation in C
	- malloc: most used memory allocation -> malloc(sizeof(.....));
	- calloc: does the same thing as malloc, except it take in two arguements whereas malloc only take one arguement. -> calloc(sizeof(num), sizeof(size)); where num is the number of the data type and size is the data type size.
	- realloc: it take in two arguements. It changes the size of the memory block pointed to by ptr. 
...
Starvation vs. Deadlock vs. Livelock:
	...
	- Livelock: a risk with some algorithms that detect and recover from deadlock. If more than one process takes action, the deadlock detection algorithm can be repeatedly triggered.
