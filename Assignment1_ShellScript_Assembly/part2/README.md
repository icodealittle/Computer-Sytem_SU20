...
Ans: Line 15 in seg.c 

3. What is the fix for the bug?

Ans: Replace the directory with an existed one.
...
Pro:
1. The static instruction count is the number of instruction that the prgram had. The tool can anlyze the program without the need to 
run the program at all. It result that it is more efficient and take less time to complete.

Con:
1. When there's an error with the program, it's hard to detect it and it is an approximation. The presence of branches, some instructions may not be able to executed at all. This lead to a huge difference from the actual number of instructions being executed.
...
Pro:
1. The dynamic instruction count is the exact number of intstruction that able to be executed by the CPU. It's the exact number for any specific program execution. Unlike statis instruction count, dynamic instruction count can detect error while the porgram is running and being executed.

Con:
1. While tatic instruction count takes less time, dynamic instruction count take more time to analyze any running program, instead of scanning it. Dynamic instruction count can also miss any bugs because they cannot check every possible path.
