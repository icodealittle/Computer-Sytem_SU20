...
void trace_friendly(int size, int firstValue, int secondValue){
  
  
  FILE* fp = fopen("trace_friendly.txt", "w");
  ...
  for (i = 0; i < size; i += 2){
    char* bin = intToBinaryString(firstValue, BITWIDTH);
	  char* bin2 = intToBinaryString(secondValue, BITWIDTH);
	  fprintf(fp,"%s\n", bin);
	  fprintf(fp,"%s\n", bin2);
	  free(bin);
  }
...
int main(){
	...
	trace_friendly(ADDRESSES, 42, 40);
