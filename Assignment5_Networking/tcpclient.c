...
int main() {
    ...
    printf("Your choosen username: ");

    char choosen_username[128];
    scanf("%[^\n]", choosen_username);
    getchar();

    send(client_socket, choosen_username, sizeof(char) * strlen(choosen_username), 0);
    ...
    while (1) {
        ...
        scanf("%[^\n]", userCommand);
        getchar();
        send(client_socket, userCommand, sizeof(char) * strlen(userCommand), 0);
